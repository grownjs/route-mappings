...
function reduceHandlers(node, parent, resource) {
  ...
  node.handler = node._isNamespace && node.as ? node.as.split('.') : node.handler || node.path.substr(1) || [];
  node.handler = (Array.isArray(node.handler) ? node.handler : [node.handler]).slice();
  ...
  if (!resource) {
    if (node.as) {
      node.handler = parent.concat(node.as);
    } else {
      var _offset = parent.indexOf(node.handler[0]);

      node.handler = _offset === -1 ? parent.concat(node.handler) : parent.slice(0, _offset).concat(node.handler);
    }
  }
}
