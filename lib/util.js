  ...
  Object.keys(source).forEach(function(key) {
    if (skip.indexOf(key) === -1) {
      if (Array.isArray(source[key]) && Array.isArray(target[key])) {
        Array.prototype.push.apply(target[key], source[key].filter(function(value) {
...
function normalizeHandler(path, state, params, method) {
  ...
  return mergeProps(state.PRIVATE_PROPERTIES, state, params);
}
...
function reduceResources(node, parent) {
  ...
  if (parent && node._isResource) {
    node.path = '/' + parent.join('/') + (node.path !== '/' ? node.path : '');
  }
}
