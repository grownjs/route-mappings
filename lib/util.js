...
function normalizeHandler(path, state, params, method) {
  ...
  if (typeof params.to === 'string') {
    params.handler = params.to.split(/\W+/);
  }

  if (Array.isArray(params.handler) && params.handler[0] && params.handler[1]) {
    params._resourceName = params.handler[0];
    params._actionName = params.handler[1];
  }
