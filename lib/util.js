...
function normalizeHandler(path, state, params, method) {
  ...
  var _fixedPath = normalizeRoute(state.PARAMS_PATTERN, path).split('/');

  if (typeof params === 'undefined') {
    params = { handler: _fixedPath[0] ? _fixedPath : [] };
  }
  ...
  if (typeof params.handler !== 'function' && !Array.isArray(params.handler)) {
    params.handler = params.handler ? [params.handler] : [];
  }
  ...
  if (typeof params.to === 'string') {
    ...
    if (!params.handler.length) {
      params.handler = _fixedPath[0] ? _fixedPath : _parts;
    }
  ...
  if (params.as) {
    params.handler = params.as.split('.');
  }
...
function reduceHandlers(node, parent, resource) {
  ...
  var _fixedHandler = node.handler || [];

  if (!_fixedHandler.length && node.path !== '/') {
    _fixedHandler = [node.path.substr(1)];
  }

  node.handler = node.as ? node.as.split('.') : _fixedHandler;

  var _offset = parent.indexOf(node.handler[0]);

  if (_offset > -1) {
    parent = parent.slice(0, _offset);
  }

  // avoid iterating already normalized handlers
  if (node.as) {
    if (node._isResourceMapper || (_fixedHandler.length && resource && node._resourceName !== _fixedHandler[0])) {
      return;
    }
  }
  ...
  _offset = parent.indexOf(_fixedHandler[0]);

  if (_offset > -1) {
    parent.splice(_offset);
  }

  if (parent.length) {
    node.handler = parent.concat(_fixedHandler);

    if (node._resourceName && parent.indexOf(node._resourceName) === -1) {
      node._resourceName = parent.concat(node._resourceName).join('.');
    }
  }
}
