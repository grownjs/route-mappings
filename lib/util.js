...
function reduceResources(node, parent) {
  if (node._resourceKey) {
    node.tree.forEach(function (route) {
      reduceResources(route, node._resourceKey);
    });
  }

  if (parent && node.tree) {
    node.tree.forEach(function (subroute) {
      reduceResources(subroute, parent);
    });
  }

  if (parent && node._isResource) {
    node.path = '/' + parent + (node.path !== '/' ? node.path : '');
  }
}

function reduceHandlers(node, parentHandler, resourceHandler) {
  ...
  if (node.tree) {
    node.tree.forEach(function(route) {
      var _fixedParent = node.handler;

      // TODO: this may be configurable?
      if (!resourceHandler || !route._isResource) {
        _fixedParent = parentHandler.concat(_fixedParent);
      }

      reduceHandlers(route, _fixedParent, node._isResource);
    });
  } else {
    var action = node._isAction ? node.handler.pop() : null;
    var _handler = node.handler.slice();

    node.handler = parentHandler;

    if (action && node.handler[node.handler.length - 1] !== action) {
      node.handler.push(action);
    } else {
      var _offset = node.handler.indexOf(_handler[0]);

      if (_offset > -1) {
        node.handler.splice(_offset, node.handler.length - _offset);
      }

      node.handler = node.handler.concat(_handler);
    }
  }
}
