...
function reduceResources(node, parent) {
  if (node.tree) {
    node.tree.forEach(function (subroute) {
      ...
      if (node._isNamespace && subroute._isResource) {
        _fixedParent = node._resourceKey;
      }

      reduceResources(subroute, _fixedParent);
    });
  }

  if (parent) {
    if (node._isResource && node.path.indexOf(parent) === -1) {
      node.path = '/' + parent + node.path;
    }
  }
}
...
function reduceHandlers(node, parent, resource) {
  ...
  if (node.tree) {
    node.tree.forEach(function(route) {
      ...
      if (!resource || !route._isResource) {
        _fixedParent = parent.concat(_fixedParent);
      }
