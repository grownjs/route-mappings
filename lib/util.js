    ...
    if (parentNode) {
      ...
      if (item.route.handler.length === 1) {
        item.route.handler = (parentNode.handler || [])
          .concat(item.route.handler[0] ? item.route.handler : [fixedKey(item.route.path)]);
      }

      if (parentNode.group === 'resource' || parentNode.group === 'resources') {
        item.route.path = `${parentNode.path}/:${
          dasherize(parentNode.handler[parentNode.handler.length - 1])
        }_id${item.route.path !== '/' ? item.route.path : ''}`;
      } else {
