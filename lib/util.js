...
function reduceHandlers(node, parent) {
  var _fixedHandler = node.as ? node.as.split('.') : node.handler || [];

  if (node.as && node._resourceName) {
    // prevents extra handlers on aliasing
    return;
  }
  ...
  if (node.tree) {
    node.tree.forEach(function(route) {
      reduceHandlers(route, parent.indexOf(_fixedHandler[0]) === -1 ? parent.concat(_fixedHandler) : parent);
    });
  }

  var _offset = parent.indexOf(_fixedHandler[0]);

  if (_offset === -1) {
    node.handler = parent.concat(_fixedHandler);
  } else {
    node.handler = parent.slice(0, _offset).concat(_fixedHandler);
  }
}
