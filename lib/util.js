...
function normalizeHandler(path, state, params, method) {
  ...
  var _fixedPath = normalizeRoute(state.PARAMS_PATTERN, path);

  if (typeof params === 'undefined') {
    params = { handler: _fixedPath || [] };
  }

  if (typeof params === 'string') {
    params = params.split(/\W+/);
  }
  ...
  if (typeof params.to === 'string') {
    ...
    if (!params.handler.length) {
      params.handler = _fixedPath ? [_fixedPath] : _parts;
    }
...
function reduceHandlers(node, parent) {
  parent = parent || [];
  ...
  if (node.as) {
    if (node._isResourceMapper || (_fixedHandler.length && node._resourceName && node._resourceName !== _fixedHandler[0])) {
      return;
    }
  }
  ...
  if (node.tree) {
    node.tree.forEach(function(route) {
      reduceHandlers(route, parent.concat(node.handler));
    });
  }
