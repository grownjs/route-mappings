...
function compileRoutes(tree, currentPath) {
  ...
  tree.forEach(function(route) {
    ...
    if (route.tree) {
      compileRoutes(route.tree, fixedPath).forEach(function(subroute) {
        routes.push(subroute);
      });
    } else {
      var fixedRoute = util.omit(route, ['tree']);

      fixedRoute.path = util.dasherize(fixedPath.join('') || '/');
      ...
      if (fixedRoute.as && keys.indexOf(fixedRoute.as) === -1) {
        keys.push(fixedRoute.as);
      }
