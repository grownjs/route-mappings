...
function compileMappings(state, currentPath) {
  ...
  compileRoutes(state, currentPath).forEach(function(route) {
    ...
    if (!alias && Array.isArray(route.handler)) {
      ...
      alias = fixedHandler.reduce(function(acc, path, index) {
        if (fixedAction === 'index' && index === fixedHandler.length - 1) {
          path = util.ucfirst(pluralize(path));
        } else {
          path = singularize(path);
      ...
      if (fixedAction !== 'index') {
        alias = fixedAction + util.ucfirst(alias);
      }
    }
