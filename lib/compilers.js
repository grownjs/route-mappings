...
function compileTree(state) {
  ...
  state.TREE.forEach(function(node) {
    ...
    if (typeof node.handler === 'function') {
      ...
      fixedNode.handler = Array.isArray(fixedNode.handler) ? fixedNode.handler : [fixedNode.handler];
    }
...
function compileRoutes(tree, state, currentPath) {
  ...
  tree.forEach(function(route) {
    ...
    if (route.tree) {
      ...
      var fixedPath = fixedPath.join('') || '/';

      fixedRoute.path = util.dasherize(fixedPath);

      var keys = fixedRoute.handler.slice();
      ...
      if (!fixedRoute.as) {
        if (!keys.length && fixedPath !== '/') {
          keys.push(util.normalizeRoute(state.PARAMS_PATTERN, fixedPath));
        }

        fixedRoute.as = keys.join('.') || 'index';
      } else {
        var _offset = keys.indexOf(fixedRoute.as);

        // prefix named paths with its namespace
        fixedRoute.as = (_offset === -1 ? keys : keys.slice(0, _offset)).concat(fixedRoute.as).join('.');
      }
