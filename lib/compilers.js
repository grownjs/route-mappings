  ...
  state.TREE.forEach(function(node) {
    ...
    if (typeof node.handler === 'function') {
      ...
      if (!Array.isArray(node.handler)) {
        fixedNode.handler = [fixedNode.handler];
      }
    }
...
function compileRoutes(tree, state, currentPath) {
  ...
  tree.forEach(function(route) {
    ...
    if (route.tree) {
      ...
      fixedRoute._origPath = fixedPath.join('') || '/';
      fixedRoute.path = util.dasherize(fixedRoute._origPath);
...
function compileKeypaths(routes, state) {
  return routes.map(function(route) {
    ...
    if (!keys.length) {
      keys.push(util.normalizeRoute(state.PARAMS_PATTERN, route._origPath));
    }

    // cleanup
    delete route._origPath;

    if (!route.as) {
      route.as = keys.join('.') || 'index';
    } else {
      // prefix named paths with its namespace
      route.as = keys.concat(route.as).join('.');
    }
