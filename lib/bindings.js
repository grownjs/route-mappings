...
function bindResourcesHelper(state, routeMapper) {
  routeMapper.resources = function() {
    ...
    fixedResources.forEach(function(resourcePath) {
      ...
      var resourceMapper = routeMapper(util.omit(params, state.PRIVATE_PROPERTIES));
      ...
      Object.keys(fixedActions).forEach(function(action) {
        ...
        Object.keys(util.omit(params, ['only', 'except'])).forEach(function(key) {
          fixedParams[key] = params[key];
        });

        resourceMapper[fixedMethod](fixedPath, fixedParams);
      });
